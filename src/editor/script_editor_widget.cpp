...
// Start with the longest, keep the shortest at the end
...
ScriptEditorWidget* ScriptEditorWidget::squirrel_callback_editor = nullptr;
...
ScriptEditorWidget::ScriptEditorWidget() :
  ...
  m_err_msg()
{
  InterfaceTheme theme(Resources::editor_font, Color::BLACK, Color(.8f, .8f, .8f), 0.f);
  m_theme = InterfaceThemeSet(theme, theme, theme, theme, theme);
}

void
ScriptEditorWidget::draw(DrawingContext& context)
{
  if (!m_visible)
    return;
...
void
ScriptEditorWidget::draw_text(DrawingContext& context)
{
  auto theme = get_current_theme();
  ...
  while (pos < s.length())
  {
    ...
    else
    {
      ...
      {
        // It's a regular symbol. Check if it's an operator, else parse them one
        // character at a time.
        ...
        for (const auto& op : operators)
        {
          if (word.find(op) == 0)
          {
            pos -= (word.size() - op.size());
            word = word.substr(0, op.size());
            is_operator = true;
            break;
          }
        }
...
void
ScriptEditorWidget::update(float dt_sec)
{
  if (!m_enabled)
    return;
...
bool
ScriptEditorWidget::event(const SDL_Event& ev)
{
  if (!m_enabled)
    return false;
...
void
ScriptEditorWidget::refresh_script_validation()
{
